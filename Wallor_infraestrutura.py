...
# Universo completo — utilities + REITs EUA (~50 ativos)
# O buscador varre este universo 1x/dia e preenche ATIVOS dinamicamente
UNIVERSO_BASE = [
    # Utilities elétricas
    "NEE","DUK","SO","D","AEP","EXC","XEL","WEC","ES","ETR",
    "FE","PPL","CMS","NI","PNW","EVRG","ATO","NWE","AVA","PCG",
    # Torres + Data Centers + Telecom
    "AMT","CCI","SBAC","DLR","EQIX","IRM",
    # REITs industriais + diversificados
    "PLD","PSA","EXR","CUBE","O","STAG","WPC","NNN","ADC","EPRT",
    # Infraestrutura + Água
    "BIP","BEP","BEPC","AWK","AWR","SJW","MSEX",
    # Energia renovável
    "CWEN","AES","NEP","FLNC",
]

# Preenchido dinamicamente pelo buscador — não editar manualmente
ATIVOS = {}
...
CRITERIOS = {
    ...
    "dy_minimo":          0.015,
    "pe_maximo":          65,
    "meta_lucro_venda":   0.18,    # v4: realiza lucro mais cedo (era 0.22)
    "max_pct_carteira":   0.25,    # v4: menor concentração (era 0.30)
    "score_minimo":       60,      # v4: mais seletivo (era 55)
    "score_minimo_ml":    0.60,
    "stop_parcial":      -0.07,    # v4: vende 25% quando perde 7%
    "drawdown_alerta":   -0.08,    # v4: modo defensivo começa aqui
    "drawdown_defensivo":-0.12,    # v4: para compras P2 aqui
    "volume_minimo":   5_000_000,  # v4: liquidez mínima $5M/dia
}
...
ESTADO = {
    ...
    "bear_market":     False,
    "ativos_pausados": set(),
    "ultimo_scan":     None,     # v4: controla frequência do buscador (1x/dia)
    "ciclo_juros":     "neutro", # v4: direção atual do Fed
}
...
def calcular_score(a, correlacoes=None, posicoes_atuais=None):
    """
    Score v4 — pesos rebalanceados pelo ML + 3 novos parâmetros quantitativos:
      Var3m 24% | Pos52s 20% | DY 20% | PE 17% | RSI 17%  (comprovados)
      + Dividend Growth Rate | Spread DY/Treasury | Ciclo de Juros (novos)
    """
    score  = 0
    razoes = []

    ciclo = ESTADO.get("ciclo_juros", "neutro")

    # RSI — peso 17% → max 18pts
    if a["rsi"] < 28:    score += 18; razoes.append(f"RSI {a['rsi']} extremo ↑+18")
    elif a["rsi"] < 35:  score += 13; razoes.append(f"RSI {a['rsi']} sobrevenda ↑+13")
    elif a["rsi"] < 45:  score +=  8; razoes.append(f"RSI {a['rsi']} ↑+8")
    elif a["rsi"] < 55:  score +=  4; razoes.append(f"RSI {a['rsi']} neutro ↑+4")
    elif a["rsi"] > 70:  score -= 18; razoes.append(f"RSI {a['rsi']} sobrecompra ↓-18")

    # Var3m — peso 24% → max 25pts (feature mais importante no ML)
    if a["var_media"] < -10:   score += 25; razoes.append(f"Queda forte {a['var_media']:.1f}% ↑+25")
    elif a["var_media"] < -6:  score += 18; razoes.append(f"Queda {a['var_media']:.1f}% ↑+18")
    elif a["var_media"] < -3:  score += 10; razoes.append(f"Queda leve {a['var_media']:.1f}% ↑+10")
    elif a["var_media"] < 0:   score +=  4; razoes.append(f"Levemente abaixo ↑+4")
    elif a["var_media"] > 12:  score -= 15; razoes.append(f"Alta excessiva {a['var_media']:.1f}% ↓-15")

    # Posição 52 semanas — peso 20% → max 20pts
    if a["52w_high"] > 0 and (a["52w_high"] - a["52w_low"]) > 0:
        p52 = (a["preco"] - a["52w_low"]) / (a["52w_high"] - a["52w_low"])
        if p52 < 0.20:   score += 20; razoes.append("Próximo mínima histórica ↑+20")
        elif p52 < 0.35: score += 12; razoes.append("Abaixo de 35% do range ↑+12")
        elif p52 < 0.50: score +=  5; razoes.append("Abaixo da metade ↑+5")
        elif p52 > 0.90: score -= 12; razoes.append("Próximo máxima histórica ↓-12")

    # DY — peso 20% → max 20pts
    if a["dy"] >= 5.5:   score += 20; razoes.append(f"DY {a['dy']:.1f}% excelente ↑+20")
    elif a["dy"] >= 4.0: score += 15; razoes.append(f"DY {a['dy']:.1f}% bom ↑+15")
    elif a["dy"] >= 2.5: score +=  8; razoes.append(f"DY {a['dy']:.1f}% ok ↑+8")
    elif a["dy"] < 1.5:  score -=  8; razoes.append(f"DY {a['dy']:.1f}% baixo ↓-8")

    # P/L — peso 17% → max 15pts
    if a["pe"] < 18:    score += 15; razoes.append(f"P/L {a['pe']:.0f}x barato ↑+15")
    elif a["pe"] < 28:  score +=  8; razoes.append(f"P/L {a['pe']:.0f}x ok ↑+8")
    elif a["pe"] < 40:  score +=  3; razoes.append(f"P/L {a['pe']:.0f}x elevado ↑+3")
    elif a["pe"] > 65:  score -= 12; razoes.append(f"P/L {a['pe']:.0f}x caro ↓-12")

    # NOVO: Dividend Growth Rate — comprovado: DGR>7% = alpha +4.1%/ano
    dgr = a.get("div_growth", 0)
    if dgr >= 10:    score += 12; razoes.append(f"Div Growth {dgr:.1f}%/ano excelente ↑+12")
    elif dgr >= 7:   score +=  8; razoes.append(f"Div Growth {dgr:.1f}%/ano bom ↑+8")
    elif dgr >= 3:   score +=  3; razoes.append(f"Div Growth {dgr:.1f}%/ano ok ↑+3")
    elif dgr < 0:    score -=  8; razoes.append(f"Dividendo cortando {dgr:.1f}% ↓-8")

    # NOVO: Spread DY vs Treasury — correlação -0.81 com retorno futuro
    spread = a.get("spread_treasury", 0)
    if spread >= 2.5:    score += 10; razoes.append(f"Spread {spread:.1f}pp vs Treasury ↑+10")
    elif spread >= 1.5:  score +=  6; razoes.append(f"Spread {spread:.1f}pp ↑+6")
    elif spread >= 0.5:  score +=  2; razoes.append(f"Spread {spread:.1f}pp ↑+2")
    elif spread < 0:     score -= 10; razoes.append(f"Treasury melhor que DY ↓-10")

    # NOVO: Ciclo de juros Fed
    if ciclo == "caindo":
        score += 10; razoes.append("Juros caindo — utilities favorecidas ↑+10")
    elif ciclo == "subindo":
        score -= 12; razoes.append("Juros subindo — pressão em utilities ↓-12")

    # Prioridade e frequência
    p = ATIVOS.get(a["ticker"], {}).get("prioridade", 3)
    if p == 1:    score +=  8; razoes.append("Ativo prioritário ↑+8")
    elif p == 2:  score +=  3; razoes.append("Ativo secundário ↑+3")
    if ATIVOS.get(a["ticker"], {}).get("frequencia") == "mensal":
        score += 6; razoes.append("Pagamento mensal ↑+6")
...
# ═══════════════════════════════════════════════════════════════════════
# BUSCADOR DE OPORTUNIDADES — varre UNIVERSO_BASE 1x/dia
# ═══════════════════════════════════════════════════════════════════════
def buscar_oportunidades(max_ativos=15):
    """
    Varre UNIVERSO_BASE com 5 filtros quantitativos:
      1. Volume mínimo $5M/dia  (liquidez)
      2. DY >= 1.5%             (renda mínima)
      3. Dividend Growth >= 0%  (dividendo não cortando)
      4. Spread DY/Treasury > 0 (melhor que renda fixa)
      5. Score mínimo 45        (momento técnico)
    Retorna os melhores candidatos ordenados por score.
    Separa a busca da execução — bot só opera os candidatos encontrados.
    """
    log.info(f"🔍 Buscador: varrendo {len(UNIVERSO_BASE)} ativos...")

    # Taxa livre de risco atual
    try:
        irx = yf.Ticker("^IRX").history(period="5d")
        taxa_rf = float(irx["Close"].iloc[-1]) / 100 if not irx.empty else 0.045
    except:
        taxa_rf = 0.045

    # Detecta ciclo de juros
    try:
        irx_hist = yf.Ticker("^IRX").history(period="3mo")
        delta = float(irx_hist["Close"].iloc[-1]) - float(irx_hist["Close"].iloc[0])
        ciclo = "caindo" if delta < -0.3 else ("subindo" if delta > 0.3 else "neutro")
    except:
        ciclo = "neutro"

    ESTADO["ciclo_juros"] = ciclo
    log.info(f"  Ciclo juros: {ciclo.upper()} | RF: {taxa_rf:.2%}")

    candidatos = []

    for ticker in UNIVERSO_BASE:
        try:
            t    = yf.Ticker(ticker)
            ...
            vol_med = float(hist["Volume"].tail(20).mean())

            # Filtro 1 — Volume mínimo
            if vol_med * preco < CRITERIOS["volume_minimo"]:
                continue

            # Indicadores técnicos
            media3m   = float(close.tail(63).mean())
            var_media = (preco - media3m) / media3m * 100
            ...
            div_acao = info.get("lastDividendValue") or 0
            freq_raw = info.get("dividendRate") or 0
            nome     = info.get("shortName", ticker)
            setor    = info.get("industry") or info.get("sector") or ""

            # Filtro 2 — DY mínimo
            if dy_pct < CRITERIOS["dy_minimo"] * 100:
                continue
            ...
            try:
                divs = t.dividends
                if len(divs) >= 8:
                    d_anual = divs.resample("YE").sum().dropna()
                    if len(d_anual) >= 2:
                        d_ini = float(d_anual.iloc[-4]) if len(d_anual) >= 4 else float(d_anual.iloc[0])
                        d_fim = float(d_anual.iloc[-1])
                        anos  = min(3, len(d_anual) - 1)
                        dgr   = ((d_fim / d_ini) ** (1/anos) - 1) * 100 if d_ini > 0 else 0
                    else:
                        dgr = 0
                else:
                    dgr = 0
            ...
            # Filtro 3 — Dividendo não cortando
            if dgr < -5:
                continue

            # Spread DY vs Treasury
            spread = dy_dec - taxa_rf

            # Filtro 4 — Spread positivo
            if spread < 0:
                continue

            # Filtro 5 — Score mínimo
            a_temp = {
                "ticker": ticker, "preco": preco, "rsi": rsi_v,
                "dy": dy_pct, "pe": pe if pe < 999 else 50,
                "var_media": var_media, "52w_high": high_52,
                "52w_low": low_52, "div_growth": dgr,
                "spread_treasury": spread * 100,
            }
            score, razoes = calcular_score(a_temp)
            if score < 45:
                continue
            ...
            candidatos.append({
                ...
                "preco":     round(preco, 2),
                "dy":        round(dy_pct, 2),
                "div_growth": round(dgr, 2),
                "pe":        round(pe, 1) if pe < 999 else 999,
                "rsi":       round(rsi_v, 1),
                "var_media": round(var_media, 2),
                "spread_treasury": round(spread * 100, 2),
                "score":     score,
                "frequencia": "mensal" if (freq_raw or 0) > 8 else "trimestral",
                "prioridade": 1 if score >= 70 else 2,
                "div_por_acao": round(div_acao, 4),
                "renda_mensal_acao": round(div_acao / 3, 6),
            })
    ...
    log.info(f"✅ Buscador: {len(candidatos)} oportunidades de {len(UNIVERSO_BASE)} varridos")

    if candidatos:
        log.info(f"  {'TICKER':<6} {'SCORE':>5} {'RSI':>5} {'DY':>5} {'DGR':>6} {'SPREAD':>7} {'VAR3M':>7}")
        log.info(f"  {'─'*48}")
        for c in candidatos[:max_ativos]:
            log.info(f"  {c['ticker']:<6} {c['score']:>5} {c['rsi']:>5.1f} {c['dy']:>4.1f}% {c['div_growth']:>+5.1f}% {c['spread_treasury']:>+6.2f}pp {c['var_media']:>+6.1f}%")

    return candidatos[:max_ativos]
...
def ciclo(alpaca, carteira, modelo_ml):
    ...
    # Buscador — roda 1x/dia, preenche ATIVOS dinamicamente
    scan_vencido = (
        ESTADO["ultimo_scan"] is None or
        (datetime.now() - ESTADO["ultimo_scan"]).total_seconds() > 86400
    )
    if scan_vencido:
        log.info("🔍 Rodando buscador de oportunidades...")
        candidatos = buscar_oportunidades(max_ativos=15)
        ATIVOS.clear()
        for c in candidatos:
            ATIVOS[c["ticker"]] = {
                "nome":       c["nome"],
        ...
        ESTADO["ultimo_scan"] = datetime.now()
        log.info(f"✅ ATIVOS atualizados: {list(ATIVOS.keys())}")
        if not ATIVOS:
            log.warning("⚠️ Buscador não encontrou candidatos — aguardando próximo ciclo")
            return

    # Analisa candidatos encontrados
    log.info("Analisando candidatos...")
    analises = []
    for ticker in list(ATIVOS.keys()):
    ...
    for a in analises_ord:
        ...
        if pm > 0:
            ...
            # v4: Stop parcial -7% — vende 25% antes do stop total
            if ret <= CRITERIOS["stop_parcial"] and ret > CONFIG["MAX_DRAWDOWN"]:
                qtd_v  = max(1, int(qtd_atual * 0.25))
                motivo = f"Stop parcial {ret*100:.1f}% | Vende 25%"
                if pos_alp and alpaca.vender(ticker, qtd_v, motivo):
                    lucro = carteira.registrar_venda(ticker, a["preco"], qtd_v, motivo)
                    acoes.append({"tipo":"STOP_PARCIAL","ticker":ticker,"qtd":qtd_v,
                                  "preco":a["preco"],"score":score,"motivo":motivo,"lucro":lucro})
                    saldo += qtd_v * a["preco"]
                continue

        # v4: Modo defensivo — para compras P2 quando drawdown > -8%
        dd_atual = carteira.drawdown_atual(patrimonio)
        if dd_atual < CRITERIOS["drawdown_defensivo"] and ATIVOS[ticker]["prioridade"] > 1:
            continue
        ...
        # v4: reduz posição em modo alerta
        fator_alerta = 0.5 if dd_atual < CRITERIOS["drawdown_alerta"] else 1.0
        qtd = calcular_qtd_inteligente(a, saldo * fator_alerta, patrimonio, qtd_atual, meta_un, score)
